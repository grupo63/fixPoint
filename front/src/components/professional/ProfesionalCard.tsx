...
export default function ProfessionalCard({ pro }: Props) {
  ...
  const loggedIsThisProfessional = isPro && user?.professionalId && user.professionalId === pro.id

  // Imagen
  const imgSrc = useMemo(
    () => pro.profileImg?.trim?.() || pro.user?.profileImage?.trim?.() || "/placeholder.png",
      ...
      <div className="relative bg-white rounded-xl p-6 shadow-lg border border-gray-100 w-full max-w-sm hover:shadow-xl transition-all duration-200 hover:border-gray-200">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm font-medium text-orange-600 mb-2">{pro.speciality}</p>
            <p className="text-sm text-gray-600 line-clamp-2 leading-relaxed">{pro.aboutMe}</p>
          </div>
