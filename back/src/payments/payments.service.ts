...
export class PaymentsService {
  ...
  async applyCheckoutCompleted(session: Stripe.Checkout.Session) {
    ...
    if (session.mode === 'subscription') {
      ...
      // Pedimos a Stripe que incluya la "última factura" en la respuesta de la suscripción.
      ...
      if (!localSub) {
        ...
        const periodEndTimestamp = invoice.period_end; // La factura SÍ tiene esta fecha.
    ...
    this.logger.log(`Pago ${payment.id} actualizado correctamente al estado: ${payment.status}`);
  }
  async createCheckoutPaymentSession(dto: CreateCheckoutPaymentDto) {
    const session = await this.stripe.checkout.sessions.create({
      mode: 'payment',
      payment_method_types: ['card'],
      line_items: [{ price_data: { currency: dto.currency, product_data: { name: dto.description }, unit_amount: dto.amount }, quantity: 1 }],
  ...
  async upsertFromInvoice(invoice: Stripe.Invoice) {
    ...
    if (typeof subId === 'string') (existing as any).stripeSubscriptionId = subId;
  ...
  async getCheckoutSession(sessionId: string) { return this.stripe.checkout.sessions.retrieve(sessionId); }
  async refund(dto: RefundPaymentDto) { return await this.stripe.refunds.create({ payment_intent: dto.paymentIntentId, amount: dto.amount }); }
}
